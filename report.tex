...
\begin{tabular}{ll}
\textbf{Sinh viên thực hiện:} & (Điền tên thành viên 1) -- (MSSV) \\
                               & (Điền tên thành viên 2) -- (MSSV) \\
                               & (Điền tên thành viên 3) -- (MSSV) \\[0.5cm]
...
int priority;       // Queue hiện tại (0=cao nhất, NMLFQ-1=thấp nhất)
int ticks_used;     // Số ticks đã dùng trong time slice hiện tại
int ticks_total;    // Tổng ticks đã dùng (thống kê)
uint64 last_run_time; // Thời điểm chạy lần cuối
\end{lstlisting}
...
struct spinlock mlfq_lock;   // Lock bảo vệ mlfq_ticks
\end{lstlisting}
...
void yield(void) {
  ...
  // Cập nhật bộ đếm toàn cục cho priority boost
  acquire(&mlfq_lock);
  mlfq_ticks++;
  release(&mlfq_lock);

  // Kiểm tra đã dùng hết time slice chưa
  int time_slice = get_time_slice(p->priority);
  if(p->ticks_used >= time_slice) {
    // Demote xuống queue thấp hơn
...
void sleep(void *chan, struct spinlock *lk) {
  ...
  // I/O-bound: yield sớm, reset ticks (không bị demote)
  p->ticks_used = 0;
...
void scheduler(void) {
  ...
  for(;;) {
    ...
    // 2. MLFQ: Tim process uu tien cao nhat
    selected = 0;
...
\subsection{Web-based UI}

\textit{(Phần này đang trong quá trình phát triển trên nhánh \texttt{feature/web-UI}. Bao gồm TUI monitor với ANSI colors và các chương trình test bổ sung. Sẽ được hoàn thiện trong giai đoạn tiếp theo.)}

\subsection{Kịch bản Demo}

\subsubsection{CPU-bound process}
Chạy \texttt{cpu\_bound}: tiến trình thực hiện tính toán nặng, dùng hết quantum $\Rightarrow$ bị demote từ Queue 0 $\rightarrow$ Queue 1 $\rightarrow$ Queue 2.

\subsubsection{I/O-bound process}
Chạy \texttt{io\_bound}: tiến trình sleep thường xuyên, yield sớm $\Rightarrow$ giữ nguyên ở Queue 0.

\subsubsection{Mixed workload}
Chạy \texttt{schedtest} hoặc \texttt{demo}: tạo đồng thời cả hai loại, quan sát MLFQ tự động phân biệt và ưu tiên I/O-bound.
...
\subsection{So sánh với scheduler thực tế}
